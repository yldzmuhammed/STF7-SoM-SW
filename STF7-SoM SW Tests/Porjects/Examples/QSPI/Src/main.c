...
int main(void)
{
  ...
  BSP_GPIOCLK_Enable();
	...
	for(i = 0; i < W25Q128FV_FLASH_SIZE; i += W25Q128FV_SUBSECTOR_SIZE)
		if(BSP_QSPI_Erase_Sector(i) != QSPI_OK)
			__breakpoint(0);
	
	for(i = 0; i < W25Q128FV_FLASH_SIZE; i += W25Q128FV_SECTOR_SIZE)
		if(BSP_QSPI_Erase_Block(i, BLOCKSIZE_64KB) != QSPI_OK)
			__breakpoint(0);
