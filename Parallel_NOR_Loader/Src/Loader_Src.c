...
{	
  ...
  Addr = Address % W25Q128FV_PAGE_SIZE;
  count = W25Q128FV_PAGE_SIZE - Addr;
  NumOfPage =  Size / W25Q128FV_PAGE_SIZE;
  NumOfSingle = Size % W25Q128FV_PAGE_SIZE;
  ...
  if (Addr == 0) /*!< Address is QSPI_PAGESIZE aligned  */
  {
    ...
    else /*!< Size > QSPI_PAGESIZE */
    {
      while (NumOfPage--)
      {
        ...
        QSPI_WritePage(Address, QSPI_DataNum, buffer);
        Address +=  W25Q128FV_PAGE_SIZE;
        buffer += W25Q128FV_PAGE_SIZE;
      }
...
/**
  ...
  * Note: Optional for all types of device
  */
KeepInCompilation uint64_t SYSVerify (uint32_t MemoryAddr, uint32_t RAMBufferAddr, uint32_t Size, uint32_t missalignement)
{
...
void QSPI_WriteEnable(QSPI_HandleTypeDef *hqspi)
{
 ...
 HAL_QSPI_Command(&QSPIHandle, &s_command, HAL_QPSI_TIMEOUT_DEFAULT_VALUE);
  ...
  s_command.DataMode       = QSPI_DATA_1_LINE;
	...
	HAL_QSPI_AutoPolling(&QSPIHandle, &s_command, &s_config, HAL_QPSI_TIMEOUT_DEFAULT_VALUE);
...
/**
  ...
  */
	#define QSPI_ERROR 1
#define QSPI_OK			0   
void QSPI_WritePage(uint32_t WriteAddr, uint32_t Size , uint8_t* pData)
{ 
  if (Size == 0)
    return;
//  /* Disable QSPI */
